...
<div class="w-full h-full" in:fly={$transition.pageIn} out:fly={$transition.pageOut}>
    <div class="w-full h-full flex flex-col justify-center items-center space-y-3">
        ...
        <div class="h-4/6 flex space-x-5">
            <button class="w-60 flex flex-col justify-center items-center relative bg-secondary rounded-xl space-y-2 overflow-hidden z-0 before:w-0 before:h-0 before:block before:absolute before:bg-shade/5 before:rounded-full before:transition-all before:duration-500 before:ease-cubic-out before:-z-10 hover:before:w-112 hover:before:h-112" on:click={() => (showCreateModal = true)}>
                <Icon name="create" className="w-12 h-12 drop-shadow-lg" />
                <p>Create room</p>
            </button>
            <div class="flex flex-col w-60 space-y-5">
                <button class="h-2/3 flex flex-col justify-center items-center relative bg-secondary rounded-xl space-y-2 overflow-hidden z-0 before:w-0 before:h-0 before:block before:absolute before:bg-shade/5 before:rounded-full before:transition-all before:duration-500 before:ease-cubic-out before:-z-10 hover:before:w-96 hover:before:h-96" on:click={() => (showJoinModal = true)}>
                    <Icon name="join" className="w-12 h-12 drop-shadow-lg" />
                    <p>Join room</p>
                </button>
                <button class="h-1/3 flex flex-col justify-center items-center relative bg-secondary rounded-xl space-y-2 overflow-hidden z-0 before:w-0 before:h-0 before:block before:absolute before:bg-shade/5 before:rounded-full before:transition-all before:duration-500 before:ease-cubic-out before:-z-10 hover:before:w-72 hover:before:h-72" on:click={() => page.set({ current: "settings", back: false })}>
